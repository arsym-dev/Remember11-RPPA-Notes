...
void Encode(size_t inLen, unsigned char* inData, FILE* outFile){
	...
	fputc((inLen & 0x000000FF), outFile);
	fputc((inLen & 0x0000FF00)>>8, outFile);
	fputc((inLen & 0x00FF0000)>>16, outFile);
    fputc((inLen & 0xFF000000)>>24, outFile);
...
int decode_file(char *in_filename, char *out_filename)
{
    ...
    */
    ...
    fread(filesize, 1, 3, infile);
    /* get output file length from next 3 bytes (big endian) */
    lengthout = (filesize[0] << 16 | filesize[1] << 8 | filesize[2]);

    /* read encoded data into a buffer */
    fseek(infile, 0, SEEK_END);
    lengthin = ftell(infile) - 4;
    fseek(infile, 4, SEEK_SET);
    
    unsigned char* encodedData = malloc(lengthin);
    if(encodedData == NULL){
        printf("Unable to allocate memory for input file data.\n");
        exit(7);
    }
    
    fread(encodedData, 1, lengthin, infile);
    /* decode data to output file */
    Decode(encodedData, outfile, lengthout);
    free(encodedData);
    printf("Successfully decoded file '%s' to '%s'.\n", in_filename, out_filename);
