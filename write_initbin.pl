...
sub write_object {
    ...
    if ($r eq '' || $r eq 'ESCAPED') {
        ...
        #my $temp1 = pack ('V*', @{$_[0]});
        #for (my $i = 0; $i < length ($temp1); $i=$i+4) {
        #    printf ("%02X%02X%02X%02X ", ord (substr ($temp1, $i, 1)), ord (substr ($temp1, $i+1, 1)), ord (substr ($temp1, $i+2, 1)), ord (substr ($temp1, $i+3, 1)));
        #}
    ...
    #	}
        ...
        local $_;
        foreach (@{$_[0]}) {
            ## Add double spacing between words
            my $temp = $_ =~ s/\s/  /rg;
            
            enqueue_pointer ($temp);
        }
        write_null ();
